...
class ContestLayout extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if(this.props.location.pathname !== nextProps.location.pathname && nextProps.location.pathname.endsWith('/problem')) {
      ...
      if(lastVisitedProblem) {
        this.setState({ lastVisitedProblem: parseInt(lastVisitedProblem, 10) });
      }
    }
  }

  componentDidMount() {
    const { dispatch, match, sessionStatus, } = this.props;
  ...
  render() {
    const { children, location, match, detail, serverTime } = this.props;
    const id = match.params.id;
